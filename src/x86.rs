...
struct InterruptInfo {
    ...
    // Should be aligned of 16-byte boundaries to pass the
...
global_asm!(
    ...
    inthandler_common:
       // General purpose registers (excpt rsp and rcx) 
       push r15
       push r14
       push r13
       push r12
       ...
       sub rsp, 512+8
       fxsave64[rsp]
       // 1st parameter: pinter to the saved CPU state
       mov rdi, rsp
...
impl Idt {
    pub fn new(segment_selector: u16) -> Self {
        ...
        entries[3] = IdtDescriptor::new(
            segment_selector,
            1,
            // Set DPL=3 to allow user land to make this interrupt (e.g. via int 3 op)
            IdtAttr::IntGateDPL3,
